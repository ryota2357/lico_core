...
impl Parser {
    pub(crate) fn new(input: Vec<SyntaxKind>) -> Self {
        Parser { input, pos: 0, events: Vec::new(), errors: Vec::new() }
    }
    ...
    pub(crate) fn next_non_trivia(&self) -> Option<SyntaxKind> {
        let mut pos = self.pos;
        loop {
            let Some(look) = self.input.get(pos) else {
                break None;
            };
            if *look == SyntaxKind::WHITESPACE || *look == SyntaxKind::COMMENT {
                pos += 1;
            } else {
                break Some(*look);
            }
...
enum EventRaw {
    ...
    Tombstone,
    EmptyError { message_index: u32 },
    StartError,
    FinishError { message_index: u32 },
}
...
pub(crate) enum Event {
    ...
    Token { kind: SyntaxKind },
    None,
    EmptyError { message: Cow<'static, str> },
    StartError,
    FinishError { message: Cow<'static, str> },
}
    ...
    pub(crate) fn complete(self, p: &mut Parser, kind: SyntaxKind) -> CompletedMarker {
        ...
        match &mut p.events[pos as usize] {
            slot @ EventRaw::Tombstone => {
                *slot = EventRaw::StartNode { kind, forward_parent: None }
            }
            _ => unreachable!(),
        }
